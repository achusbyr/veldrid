...
namespace Veldrid
{
    ...
    public abstract class CommandList : IDeviceResource, IDisposable
    {
        ...
        {
            SetViewport(index, ref viewport);
        }
        ...
        {
            fixed (T* ptr = &source) UpdateBuffer(buffer, bufferOffsetInBytes, (IntPtr)ptr, Util.USizeOf<T>());
        }
            ...
            uint layerCount)
        {
            ...
            uint effectiveDstArrayLayers = (destination.Usage & TextureUsage.Cubemap) != 0
                ? destination.ArrayLayers * 6
                : destination.ArrayLayers;
            if (dstBaseArrayLayer + layerCount > effectiveDstArrayLayers) throw new VeldridException("An invalid mip range was given for the destination Texture.");
#endif
            CopyTextureCore(
                source,
                srcX, srcY, srcZ,
        ...
        public void GenerateMipmaps(Texture texture)
        {
            ...
            if (texture.MipLevels > 1) GenerateMipmapsCore(texture);
        }
