...
namespace Veldrid
{
    internal static class ValidationHelpers
    {
        ...
        internal static void ValidateResourceSet(GraphicsDevice gd, ref ResourceSetDescription description)
        {
            ...
            for (uint i = 0; i < elements.Length; i++) validateResourceKind(elements[i].Kind, resources[i], i);

            for (int i = 0; i < description.Layout.Description.Elements.Length; i++)
            {
                var element = description.Layout.Description.Elements[i];
                ...
                {
                    var range = Util.GetBufferRange(description.BoundResources[i], 0);
        ...
        private static void validateResourceKind(ResourceKind kind, IBindableResource resource, uint slot)
        {
            switch (kind)
            {
                ...
                {
                    if (!Util.GetDeviceBuffer(resource, out var b)
